...
int main(int argc, char **argv, char **envp)
{
    ...
    if (msgrcv(msgid, &msg_shared_inib, MSG_SIZE_IDENT_INT, 10, MSG_NOERROR) == -1)
    ...
    int ENERGY_EXPLODE_THRESHOLD_INT = atoi(ENERGY_EXPLODE_THRESHOLD_ENV);
    ...
    while (1)
    {
        ...
        {
            pause();
            livello_energia = ((float)(scissioni[0].energia_prodotta - scissioni[0].energia_consumata) / ENERGY_EXPLODE_THRESHOLD_INT);
            // printf("livello_energia: %f\n", livello_energia);
            if (livello_energia < soglia_massima)
            { // tutto a posto
                while (msgrcv(msgid, &msg_energy, MSG_SIZE_IDENT_INT, 30, MSG_NOERROR | IPC_NOWAIT) != -1)
                {
            ...
            else
            {
                ...
                while (msgrcv(msgid, &lettura_identificazione_handl, MSG_SIZE_IDENT_INT, 1, MSG_NOERROR | IPC_NOWAIT) != -1)
                {
